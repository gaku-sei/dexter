...
fn App(cx: Scope<AppProps>) -> Element {
    ...
    let download_progress = use_ref(cx, HashMap::<String, u8>::new);
    ...
    use_future!(cx, |mangas_search| {
        ...
        async move {
            ...
            let received_mangas = match search(mangas_search, 50).await {
                Ok(mangas) => mangas,
                Err(err) => {
    ...
    use_future!(cx, |selected_manga_id| {
        ...
        async move {
            ...
            let received_chapters =
                match get_chapters(manga_id, 100, Vec::<String>::new(), Vec::<String>::new()).await
                {
    ...
    cx.render(rsx! {
        div {
            ...
            for (chapter_id, percent) in download_progress.read().iter() {
                Progress {
                    label: chapter_id.to_string(),
                    percent: *percent,
                }
            }
            div {
